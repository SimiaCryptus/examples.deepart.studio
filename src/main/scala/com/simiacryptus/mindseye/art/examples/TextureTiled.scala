...
class TextureTiled extends ArtSetup[Object] {
  ...
  override def description = """Creates a tiled texture based on a style."""
  ...
  override def postConfigure(log: NotebookOutput) = {
    ...
    def tiledCanvas = {
      ...
      val tensor = layer.eval((1 to (rowsAndCols*rowsAndCols)).map(_=>canvas.get()):_*).getDataAndFree.getAndFree(0)
      layer.freeRef()
      tensor
    }
    val calcFn: Seq[Int] => Layer = dims => {
