          ...
          withMonitoredGif(() => renderedCanvases.map(_ ()), delay = animationDelay) {
            try {
              log.subreport("Neurons in " + layer.name(), (sub:NotebookOutput)=>{
                (for ((list, page) <- (fromDim until toDim).grouped(8).zipWithIndex.toStream) yield {
                  sub.subreport("Page " + page, (sub2: NotebookOutput) => {
                    for (dimensionSelected <- list) yield {
                      sub2.h2(layer.name() + " " + dimensionSelected)
                      (1 to repeat).map(_ => {
                        test(layer, dimensionSelected)(sub2)
                      }).last
                    }
                  })
                }).flatten.foreach(img => renderedCanvases += (() => img))
              })
              null
            } finally {
